...
class BrandAgentEntry(AdCPBaseModel):
    ...
    url: Annotated[AnyUrl, Field(description='Agent endpoint URL (MCP or A2A)')]
    id: Annotated[
        str,
        Field(
    ...
    jwks_uri: Annotated[
        AnyUrl | None,
        Field(
            description="HTTPS URL of the agent's JWKS (RFC 7517) containing public keys used to verify artifacts this agent signs — for governance agents, the signed governance_context token. When absent, verifiers MUST default to /.well-known/jwks.json on the origin of `url`. Keys are identified by `kid` in the JWS header; JWKS MAY contain multiple keys to support rotation."
        ),
    ] = None
    scope: Annotated[
        list[ScopeEnum] | None,
